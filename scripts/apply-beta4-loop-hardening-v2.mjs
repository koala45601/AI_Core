...
if (source.includes(marker)) {
  console.log("Alpha beta4 workflow hardening already applied");
  process.exit(0);
}
if (!source.includes("alpha-beta4-agent-loop-v1")) throw new Error("ต้อง apply beta4 chat runtime v2 ก่อน");
...
replaceOnce(finalNeedle, finalReplacement, "truthful completion status");

const tmp = path + ".beta4-hardening-v2.tmp";
await fs.writeFile(tmp, source, "utf8");
await fs.rename(tmp, path);
console.log("Applied beta4 workflow hardening v2");
