...
const skills = [
  ...
  {
    objective: "ตรวจรายการคอนจากหน้าสาธารณะโดยไม่ต้อง Login แสดงสถานะเปิดขาย/กำลังจะเปิด/ขายหมด/ปิด/จบ/ยกเลิก แล้วสร้างโปรแกรม Python ที่ยืนยัน Login ก่อน Checkout และเลือกโซน แถว เลขที่นั่งจากหน้าจริง",
    success_criteria: "ไม่สร้างจนกว่าผู้ใช้เลือกคอน สร้างโปรเจกต์ Python+Playwright พร้อม run-full-loop.command ที่แยก close-sale, Login, terms, zone/image-map, row/seat preference, quantity, attendee, checkout options และยืนยันหน้า QR จากหลักฐานอย่างน้อย 3 สัญญาณ โดยไม่เก็บรหัสผ่านหรือ OTP",
    verification_scope: "7 visible + 20 hidden skill fixtures และ generated state fixtures ครอบคลุมรายการ public ที่ถูก fallback, สถานะขาย, การเลือกคอน รอบ โซน A/A1/A2 แถวและเลขที่นั่ง บัตรยืน งบ จำนวน ฟิลด์เฉพาะงาน วิธีรับบัตร วิธีจ่าย API evidence คิว Login CAPTCHA OTP และ QR handoff บน macOS host",
    skill: {
      id: "concert-ticket-purchase-assistant",
      name: "Python Bot Builder — Concert Ticket",
      description: "สร้างโปรแกรม Python+Playwright จากลิงก์และ Network/API evidence ลง Program_Create ตรวจรายการ public ได้โดยไม่ Login แต่ Full Loop ต้องยืนยัน Login รองรับ image-map, โซน/แถว/เลขที่นั่ง, หลายใบ และหยุดที่หน้า QR",
      runtime: "python",
      entrypoint: "main.py",
      dependencies: ["python-stdlib"],
      ...
      test_cases: [
        ...
        { name: "ask-seat", input: { event_url: "https://tickets.test/e/1", selected_event_id: "show-1", selected_event_name: "Test Concert", quantity: 2, seat_mode: "reserved" }, stdout_contains: "verified_event_facts", expected_files: ["ticket-assistant-plan.json"] },
        { name: "select-reserved", input: { ...verifiedTicketInput, event_url: "https://tickets.test/e/1", selected_event_id: "show-1", selected_event_name: "Test Concert", quantity: 2, schedule: "2026-09-01 19:00", sale_open_at: "2026-08-25T09:00:00+07:00", queue_open_at: "2026-08-25T08:00:00+07:00", seat_mode: "reserved", preferred_zones: ["A1"], preferred_rows: ["K"], preferred_seat_numbers: ["10-11"], seat_fallback_mode: "exact", budget: 5000, customer_name: "Test User", shipping_address: { city: "Bangkok" }, payment_method: "qr", captured_api: [{ method: "GET", url: "/api/seats" }] }, stdout_contains: "project_verified", expected_files: ["ticket-assistant-plan.json", "ticket-assistant-plan.md"] },
