...
for PORT in 4317 4318; do
  for PID in $(lsof -nP -iTCP:$PORT -sTCP:LISTEN -t 2>/dev/null || true); do
    ...
    if [[ "$CMD" == *"tool-service/server.mjs"* || "$CMD" == *"tool-service/server-wrapper-preview.mjs"* ]]; then
...
if [[ "$READY" != true ]]; then
  echo "Alpha v1.1 Host Tool Controller เปิดไม่สำเร็จ"
  echo "ดู log: $ALPHA_TOOL_ERROR_LOG_FILE"
  exit 1
fi

echo "Alpha v1.1 preview พร้อม: Mac capability + self-install controller ทำงานแล้ว"
