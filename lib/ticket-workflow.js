...
export function extractTicketPageFacts(snapshot = {}) {
  ...
  const saleStatus = allInPersonSoldOut ? "sold_out" : isClosed ? "closed" : isUpcoming ? "upcoming" : explicitOpen || saleEntryControls.length ? "open" : "unknown";
  ...
  return {
    ...
    ticket_status: allInPersonSoldOut ? "sold_out" : isClosed ? "closed" : isUpcoming ? "coming_soon" : inPersonPerformances.some((item) => item.status === "sold_out") ? "mixed_availability" : saleEntryControls.length ? "available" : "unknown",
    venue,
