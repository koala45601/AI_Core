const COMPLEX_TURN = /(ล่าสุด|วันนี้|ตอนนี้|ข่าว|ราคา|อากาศ|ค้น|เสิร์ช|เว็บ|internet|search|ไฟล์|โค้ด|โปรแกรม|สร้าง|แก้|รัน|เปิด|คลิก|ดาวน์โหลด|api|endpoint|docker|skill|สกิล|ฝึก|เรียนรู้|hack|wifi|wi-fi|password|รหัสผ่าน)/i;
...
export function classifyConversationTurn(message, options = {}) {
  ...
  if (!text || options.forceSearch === true || text.length > 120 || /https?:\/\//i.test(text) || COMPLEX_TURN.test(text)) return { route: "model", intent: "complex" };
  const matched = SOCIAL_INTENTS.find((item) => item.pattern.test(text));
  return matched ? { route: "instant", intent: matched.intent } : { route: "model", intent: "conversation" };
}
