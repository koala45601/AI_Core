...
export interface TicketScheduleCacheRecord {
  ...
  performance_options: Array<{ schedule?: string; label?: string; context_text?: string }>;
  sale_open_at: string;
...
export async function loadTicketScheduleCache(sourceUrl: string): Promise<Map<string, TicketScheduleCacheRecord>> {
  ...
  const rows = await db.prepare(`SELECT source_url, event_id, event_name, event_url, show_dates_json,
    performance_options_json, sale_open_at, queue_open_at, updated_at
    FROM ticket_event_schedule_cache WHERE source_url = ? ORDER BY updated_at DESC`).bind(sourceUrl).all<{
      source_url: string; event_id: string; event_name: string; event_url: string; show_dates_json: string;
      performance_options_json: string; sale_open_at: string; queue_open_at: string; updated_at: number;
    }>();
...
export async function saveTicketScheduleCache(records: TicketScheduleCacheRecord[]): Promise<void> {
  ...
  await db.batch(records.slice(0, 100).map((record) => db.prepare(`
    INSERT INTO ticket_event_schedule_cache
      (source_url, event_id, event_name, event_url, show_dates_json, performance_options_json, sale_open_at, queue_open_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(source_url, event_id) DO UPDATE SET
      ...
      sale_open_at=excluded.sale_open_at, queue_open_at=excluded.queue_open_at, updated_at=excluded.updated_at
  `).bind(
    ...
    record.sale_open_at, record.queue_open_at, record.updated_at,
  )));
}
