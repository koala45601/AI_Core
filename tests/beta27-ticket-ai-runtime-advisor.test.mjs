...
test("launcher rejects a stale Tool Service and synchronizes the installed ticket generator", async () => {
  ...
  try {
    ...
    await writeFile(join(temporary, "package.json"), JSON.stringify({ version: "1.1.0-beta.28" }), "utf8");
    ...
    assert.equal(manifest.generator_version, "1.1.0-beta.28");
    assert.equal(manifest.version, 28);
    const index = JSON.parse(await readFile(join(workDir, "skills-index.json"), "utf8"));
    assert.equal(index.at(-1).generator_version, "1.1.0-beta.28");
  } finally {
    await rm(temporary, { recursive: true, force: true });
  }
});
